...
def train_vanilla(args, model, train_loader, val_loader, device, vocab):
    ...
    for epoch in range(args.epochs):
        ...
        # Validation
        val_loss, val_perplexity = compute_perplexity(model, val_loader, criterion, device)
...
def train_dendritic(args, model, train_loader, val_loader, device, vocab):
    ...
    try:
        ...
        # Replace the dendritic output_projection with the original Linear layer
        # This excludes it from dendritic augmentation (final projection doesn't need dendrites)
        model.output_projection = original_output_proj
        print("  ⚠️  Output projection restored (excluded from dendritic conversion)")
    ...
    initial_dendrites = initial_params - base_params  # Dendrites added at initialization
    ...
    print(f"  Initial dendrites added: +{initial_dendrites:,} (+{initial_dendrites/base_params*100:.1f}%)\n")
    ...
    for epoch in range(args.epochs):
        ...
        if restructured:
            ...
            print(f"     Increase: +{param_increase:,} ({param_increase/current_params*100:.2f}%)")
    ...
    print(f"  Initial dendrites (at init): +{initial_dendrites:,} (+{initial_dendrites/base_params*100:.1f}%)")
    print(f"  Dynamic dendrites (during training): +{dynamic_param_increase:,}")
    print(f"  Final parameters: {final_params:,}")
    print(f"  Total increase: +{total_param_increase:,} (+{total_param_increase/base_params*100:.1f}%)")
...
def main():
    ...
    # Model type
    parser.add_argument('--model_type', type=str, default='vanilla', choices=['vanilla', 'dendritic'],
    ...
    # Logging
    parser.add_argument('--wandb_project', type=str, default='dendritic-transformer-comparison',
    ...
    if args.model_type == 'vanilla':
        model, best_ppl = train_vanilla(args, model, train_loader, val_loader, device, vocab)
    else:
        model, best_ppl = train_dendritic(args, model, train_loader, val_loader, device, vocab)
