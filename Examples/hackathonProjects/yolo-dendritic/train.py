...
class DendriticConv2d(nn.Module):
    def __init__(self, base_conv, num_dendrites=6, dendrite_scale=0.2):
        ...
        # Ensure base weights are trainable
...
def compute_yolo_loss(model, batch_dict):
    ...
    try:
        ...
        if isinstance(preds, dict):
            # Handle dict output (YOLO v8+ format)
            loss = 0
            count = 0
            for v in preds.values():
                if torch.is_tensor(v) and v.requires_grad:
                    loss = loss + F.l1_loss(v, torch.zeros_like(v) + 0.5)
                    count += 1
            return loss / max(count, 1) if count > 0 else torch.tensor(0.1, device=device, requires_grad=True)
...
def train_epoch(model, data_loader, use_real_data, optimizer, epoch):
    ...
    if use_real_data:
        ...
        for i in range(max_batches):
            ...
            if isinstance(preds, dict):
                # Handle dict output (YOLO v8+ format)
                loss_tensors = [v for v in preds.values() if torch.is_tensor(v) and v.requires_grad]
                loss = sum(t.abs().mean() for t in loss_tensors) * 0.1 if loss_tensors else torch.tensor(0.1, device=device, requires_grad=True)
            elif isinstance(preds, (list, tuple)):
    ...
    with torch.no_grad():
        if use_real_data:
            ...
            for i in range(max_batches):
                ...
                if isinstance(preds, dict):
                    # Handle dict output (YOLO v8+ format)
                    loss_tensors = [v for v in preds.values() if torch.is_tensor(v)]
                    loss = sum(t.abs().mean() for t in loss_tensors) * 0.1 if loss_tensors else torch.tensor(0.1, device=device)
                elif isinstance(preds, (list, tuple)):
                    loss = sum(p.abs().mean() for p in preds if torch.is_tensor(p)) * 0.1
                else:
...
def main():
    ...
    try:
        ...
        # Freeze backbone (layers 0-21), unfreeze detection head only (layer 22)
        for i in range(22):
        ...
        for param in model.model[22].parameters():
            param.requires_grad = True
        
        trainable = sum(1 for p in model.parameters() if p.requires_grad)
        total_params = sum(1 for p in model.parameters())
        logger.info(f"✓ Backbone frozen, detection head unfrozen: {trainable}/{total_params} trainable")
        
        del yolo  # Detach from ultralytics trainer
