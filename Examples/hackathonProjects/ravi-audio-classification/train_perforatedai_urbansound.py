"""
Train CNN14 model WITH dendrites using PerforatedAI on UrbanSound8K.
"""
import os
import json
import argparse
...
from utils.urbansound_model import CNN14UrbanSound
...
def train_perforatedai_urbansound(args):
    ...
    # ========================================================================
    print("\nInitializing CNN14 UrbanSound model with PerforatedAI...")
    num_classes = data_dict.get('num_classes', 10)
    model = CNN14UrbanSound(num_classes=num_classes)
    
    # Convert model to PAI model (adds dendrite capability to layers)
    model = UPA.initialize_pai(model, save_name="PAI_CNN14_UrbanSound")
    ...
    best_model_path = os.path.join(config.MODELS_DIR, 'pai_urbansound_best.pt')
    ...
    cm_path = os.path.join(config.MODELS_DIR, 'pai_urbansound_confusion_matrix.png')
    ...
    # ====================================================================
    baseline_results_path = os.path.join(config.MODELS_DIR, 'baseline_urbansound_results.json')
    ...
    results_path = os.path.join(config.MODELS_DIR, 'pai_urbansound_results.json')
    with open(results_path, 'w') as f:
        json.dump(results, f, indent=2)
    
    print(f"\nResults saved to: {results_path}")
    print(f"Best model saved to: {best_model_path}")
    print("PAI output graphs saved to: PAI_CNN14_UrbanSound/PAI_CNN14_UrbanSound.png")


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Train CNN14 with PerforatedAI dendrites on UrbanSound8K')
    ...
    train_perforatedai_urbansound(args)
