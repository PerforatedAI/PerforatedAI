"""
Train MobileNetV2 WITH dendrites using PerforatedAI on UrbanSound8K.
Pretrained on ImageNet, fine-tuned on audio with dendritic optimization.
"""
import os
import json
import argparse
...
def train_perforatedai_mobilenet(args):
    ...
    # ========================================================================
    print("\n" + "="*60)
    print("Initializing Pretrained MobileNetV2 with PerforatedAI...")
    print("="*60)
    ...
    print("Converting MobileNetV2 to PAI model...")
    model = UPA.initialize_pai(model, save_name="PAI_MobileNet_UrbanSound")
    
    model = model.to(device)
    print(f"Initial parameters: {UPA.count_params(model):,}")
    print("Strategy: ImageNet Transfer Learning + Dendritic Optimization")
    
    # ========================================================================
    # Setup Optimizer via PAI Tracker
    # ========================================================================
    criterion = nn.CrossEntropyLoss()
    args.lr = args.lr if args.lr is not None else 0.001
    ...
    best_model_path = os.path.join(models_dir, 'pai_mobilenet_best.pt')
    ...
    cm_path = os.path.join(models_dir, 'pai_mobilenet_confusion_matrix.png')
    ...
    # ====================================================================
    baseline_results_path = os.path.join(models_dir, 'baseline_mobilenet_results.json')
    if os.path.exists(baseline_results_path):
        print("\n" + "="*60)
        print("Comparison with Baseline MobileNetV2")
        print("="*60)
        ...
        print(f"\nBaseline MobileNetV2:  {baseline_acc:.2f}%")
        print(f"MobileNetV2 + PAI:     {pai_acc:.2f}%")
        print(f"Improvement:           {improvement:+.2f}%")
        print(f"Error Reduction:       {error_reduction:.2f}%")
    
    # Save results
    results = {
        'model': 'PAI_MobileNet_UrbanSound',
    ...
    results_path = os.path.join(models_dir, 'pai_mobilenet_results.json')
    with open(results_path, 'w') as f:
        json.dump(results, f, indent=2)
    
    print(f"\nResults saved to: {results_path}")
    print(f"Best model saved to: {best_model_path}")
    print("PAI output graphs saved to: PAI_MobileNet_UrbanSound/PAI_MobileNet_UrbanSound.png")


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Train MobileNetV2 with PerforatedAI on UrbanSound8K')
    ...
    train_perforatedai_mobilenet(args)
